...
def spez_original_script(post_text):
  ...
  system_role = '''
#ROLE
Ты копирайтер, который обрабатывает текст из транскрибированного видео файла, который может быть на любом языке(даже на корейском, китайском, арабском и тд).
Видео файл - это Instagram reels, Youtube shorts или TikTok видео.
Транскрибация может состоять из:
1. закадрового голоса(например специалист говорит о чем-то)
2. просто момента из жизни человека
...
Если язык нестандартный, все равно обработай его. Даже если это иероглифы - Ты справишься!
...
- СТОРОГО выписывай транскрибацию не меняя слова. Все что было в INPUT должно оказаться в OUTPUT!
- ВАЖНО! Если ты ничего не определил, просто выпиши INPUT в `content`!
  '''
