...
def process_data_pool(account, days=2, links=[], scheme=0):
    start_time = time.time()
    ...
    # 1) Сбор данных из Google-таблицы (если scheme=0)
    if scheme == 0:
        users_data = ggl.get_table_data_as_json(account, 'DATA')
        if not users_data:
            print('No users')
        ...
        sorted_data, sortedReelsCount = apify.instagram_scrapper_filter_sorter(reelsData, users_data)
    ...
    # Запоминаем время, потраченное на Apify
    apify_end_time = time.time()
    print(f"Время после Apify: {apify_end_time - start_time:.2f} секунд")
    ...
    output_apify_filename = f"{account['username']}_apify_{date_time_str}.json"
    save_path_apify = os.path.join(save_path, output_apify_filename)

    # Сохраняем apify-данные в JSON
    with open(save_path_apify, "w", encoding="utf-8") as file:
        json.dump(reelsData, file, ensure_ascii=False, indent=4)

    # Извлекаем удобную структуру для дальнейшей работы
    if scheme in [0, 1]:
        extracted_data = apify.extracted_reels_data_maker(sorted_data)
    else:
        extracted_data = apify.extracted_tiktok_data_maker(sorted_data)
    ...
    # ПАРАЛЛЕЛЬНЫЕ ЗАДАЧИ
    # ---------------------------
    #  - videos_worker: шаги [3,4,6,7]
    #  - shares_worker: шаг [5]

    # Для shares_worker нужно имя для финального result-файла
    result_filename = f"{account['username']}_result_{date_time_str}.json"
    save_path_result = os.path.join(save_path, result_filename)

    # Чтобы shares_worker мог работать, ему тоже нужен какой-то список reels,
    # где уже есть shortCode/ссылки и пр. 
    # Можно либо сразу передавать extracted_data, либо пустую заготовку `results`.
    # Ниже — передадим extracted_data и потом соединим.

    with ProcessPoolExecutor(max_workers=2) as executor:
        # будущее для обработки видео и GPT
        future_videos = executor.submit(
            videos_worker,
            account,
            scheme,
            extracted_data,
        ...
        # будущее для поиска репостов
        future_shares = executor.submit(
            shares_worker,
            extracted_data,   # или сразу future_videos.result() — но тогда не будет параллелизма
            result_filename,
            save_path_result
        )

        # Дожидаемся результатов
        results_videos = future_videos.result()  # список reels без shares
        results_shares = future_shares.result()  # либо список reels со shares, либо {shortCode: shares}

    # ---------------------------
    # КОМБАЙН РЕЗУЛЬТАТОВ
    # ---------------------------
    # В зависимости от того, что именно возвращает shares_worker,
    # нужно корректно объединить результаты.
    # Допустим, shares_worker возвращает тот же формат (список словарей), 
    # где внутри каждого dict есть поле 'shortCode' и добавлен 'shares'.
    # Тогда можно сделать мэп по shortCode:
    
    shares_map = {}
    for item in results_shares:
        sc = item.get('shortCode')
        if sc:
            shares_map[sc] = item.get('shares', 0)

    # Дописываем поле 'shares' в results_videos
    for reel_item in results_videos:
        sc = reel_item.get('shortCode')
        if sc in shares_map:
            reel_item['shares'] = shares_map[sc]
        else:
            reel_item['shares'] = 0  # или None

    # Итоговый список
    final_results = results_videos  # теперь в нем есть и GPT-поля, и shares

    # Сохраняем final_results
    os.makedirs(os.path.dirname(save_path_result), exist_ok=True)
    with open(save_path_result, "w", encoding="utf-8") as result_file:
        json.dump(final_results, result_file, ensure_ascii=False, indent=4)
    ...
    try:
        if scheme == 0:
            ggl.append_data_to_google_sheet(final_results, account["table_id"], 'INSTAGRAM')
        elif scheme == 1:
            ggl.append_data_to_google_sheet(final_results, account["table_id"], 'INSTAGRAM_SAVED')
        elif scheme == 2:
            ggl.append_data_to_google_sheet(final_results, account["table_id"], 'TIKTOK_SAVED', scheme=2)

        print(f"\nDONE\n\n")
        end_time = time.time()
        print(f"Общее время обработки: {end_time - start_time:.2f} секунд")

        return "Process completed successfully!"
    except Exception as e:
        print(f"Ошибка в процессе обработки в ggl: {e}")
        return "Process completed with error."
